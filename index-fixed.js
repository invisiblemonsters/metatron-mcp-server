...
function analyzeContract(source) {
  ...
  return {
    ...
    grade: findings.length === 0 ? "A" : findings.length < 3 ? "C" : "F"
  };
}
...
server.tool(
  ...
  async (params) => {
    ...
    if (!source) {
      return {
        content: [{ type: "text", text: JSON.stringify({ error: "Contract not verified" }) }],
      };
    }
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          price: PRICES["smart-contract-audit"],
        }, null, 2)
      }],
    };
  }
...
app.post("/mcp/audit", async (req, res) => {
  ...
  res.json({
    agentId: AGENT_ID,
    contract: { address, name: source?.name || "Unknown" },
    findings: analysis.findings,
    score: analysis.score,
    grade: analysis.grade,
  });
